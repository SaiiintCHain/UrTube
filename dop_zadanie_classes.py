...
class UrTube():
    ...
    def log_in(self,nickname, password):
        if nickname in self.users:
            if self.users[nickname].password == hash(password):
                self.current_user = self.users[nickname]
                print (f'Вы вошли под именем {nickname}')
        else:
    ...
    def get_videos(self, search_term):
        found_videos = [title for title in self.videos if search_term.lower() in title.lower()]
        if not found_videos:
    ...
    def watch_video(self, title):
        ...
        for i in range(1, video.duration + 1):
            print(i, end=' ', flush=True)  # flush=True для немедленного вывода
            time.sleep(1)
        print('\nКонец видео')
